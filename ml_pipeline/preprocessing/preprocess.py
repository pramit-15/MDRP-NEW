...
def preprocess_heart():
    ...
    if not os.path.exists(path):
        logger.warning(f"[Heart] SKIPPED — file not found: {path}")
        return
    ...
    joblib.dump(scaler, Config.HEART_SCALER_PATH)
    ...
    logger.info(f"[Heart] Done | shape: {processed.shape} | features: {available}")
...
def preprocess_diabetes():
    ...
    if not os.path.exists(path):
        logger.warning(f"[Diabetes] SKIPPED — file not found: {path}")
        return
    ...
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)
    joblib.dump(scaler, Config.DIABETES_SCALER_PATH)
    ...
    logger.info(f"[Diabetes] Done | shape: {processed.shape} | features: {DIABETES_COLS}")
...
def preprocess_kidney():
    ...
    if not os.path.exists(path):
        logger.warning(f"[Kidney] SKIPPED — file not found: {path}")
        return
    ...
    joblib.dump(scaler, Config.KIDNEY_SCALER_PATH)

    # ── Save the exact feature order for inference ─────────────────────────────
    joblib.dump(ALL_KIDNEY_COLS, Config.KIDNEY_FEATURES_PATH)
    ...
    logger.info(f"[Kidney] Done | shape: {processed.shape}")
    logger.info(f"         Numeric    : {numeric_available}")
    logger.info(f"         Categorical: {categoric_available}")
    logger.info(f"         Total features: {len(ALL_KIDNEY_COLS)}")
...
def preprocess_health_markers():
    ...
    if path is None:
        logger.warning("[HM] SKIPPED — health_markers_dataset.csv not found in data/raw/")
        return
    ...
    if path != dest:
        ...
        logger.info(f"[HM] Copied dataset to {dest}")

    df = pd.read_csv(dest)
    logger.info(f"[HM] Loaded {len(df)} records, {df['Condition'].isna().sum()} target NaN rows")
    ...
    joblib.dump(list(le.classes_), Config.HM_CLASSES_PATH)
    logger.info(f"[HM] Classes: {list(le.classes_)}")
    ...
    joblib.dump(HM_CANONICAL, Config.HM_FEATURES_PATH)

    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)
    joblib.dump(scaler, Config.HM_SCALER_PATH)
    ...
    logger.info(f"[HM] Done | shape: {processed.shape} | "
...
if __name__ == "__main__":
    ...
    logger.info(f"All preprocessing complete. Scalers saved to {Config.MODELS_DIR}")
